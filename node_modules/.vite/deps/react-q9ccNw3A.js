...
function serializeIframeUrl(attrs, props) {
	...
	return `${EMBED_BASE}/embed/${type}/${metaId}${((_c = props.config) == null ? void 0 : _c.preferVideo) ? "/video" : ""}?${serialize(params)}`;
}
		...
		if (this.#isInit) {
			...
			this.api.addListener("playback_update", (event) => {
				...
				if (this.#paused && (event.data.isBuffering || !event.data.isPaused)) {
					...
					if (this.#isWaiting) this.dispatchEvent(new Event("waiting"));
					else this.dispatchEvent(new Event("playing"));
					return;
				}
...
async function loadScript(src, globalName, readyFnName) {
	...
	if (globalName && self[globalName]) return Promise.resolve(self[globalName]);
...
if (globalThis.customElements && !globalThis.customElements.get("spotify-audio")) globalThis.customElements.define("spotify-audio", SpotifyAudioElement);
var spotify_audio_element_default = SpotifyAudioElement;
//#endregion
//#region node_modules/spotify-audio-element/dist/react.js
...
//# sourceMappingURL=react-q9ccNw3A.js.map