...
import { t as MediaPlayedRangesMixin } from "./dist-CzLAnXOn.js";
//#region node_modules/youtube-video-element/dist/youtube-video-element.js
...
function parseStartTime(url) {
	...
	return hasValue ? totalSeconds : void 0;
}
function getTemplateHTML(attrs, props = {}) {
	...
	if ((_a = props.config) == null ? void 0 : _a.referrerpolicy) iframeAttrs.referrerpolicy = props.config.referrerpolicy;
...
function serializeIframeUrl(attrs, props) {
	...
	if (VIDEO_MATCH_SRC.test(attrs.src)) {
		...
		return `${embedBase}/${matches2 && matches2[1]}?${serialize(params)}`;
	}
	const matches = attrs.src.match(PLAYLIST_MATCH_SRC);
	return `${embedBase}?${serialize({
		listType: "playlist",
		list: matches && matches[1],
		...params
	})}`;
}
var YoutubeVideoElement = class extends MediaPlayedRangesMixin(globalThis.HTMLElement ?? class {}) {
	...
	async load() {
		...
		this.textTracks.addEventListener("change", () => {
			...
			(_a = this.api) == null || _a.setOption("captions", "track", active ? { languageCode: active.language } : {});
		});
		...
		this.api = new YT.Player(iframe, { events: {
			onReady: () => {
				var _a;
				this.#readyState = 1;
				this.dispatchEvent(new Event("loadedmetadata"));
				this.dispatchEvent(new Event("durationchange"));
				if (this.#initialVolume !== 1) (_a = this.api) == null || _a.setVolume(this.#initialVolume * 100);
		...
		this.api.addEventListener("onStateChange", (event) => {
			...
			if (state === YT.PlayerState.PLAYING || state === YT.PlayerState.BUFFERING) {
				...
				(this.api.getOption("captions", "tracklist") || []).forEach((t) => {
					if (![...this.textTracks].some((tt) => tt.language === t.languageCode)) this.#textTracksVideo.addTextTrack("subtitles", t.displayName, t.languageCode);
					this.textTracks = this.#textTracksVideo.textTracks;
				});
				this.dispatchEvent(new Event("loadstart"));
			}
			if (state === YT.PlayerState.PLAYING) {
				if (this.seeking) {
					this.#seeking = false;
					(_a = this.#seekComplete) == null || _a.resolve();
		...
		this.api.addEventListener("onVolumeChange", () => {
			var _a;
			this.#initialVolume = ((_a = this.api) == null ? void 0 : _a.getVolume()) / 100;
			this.dispatchEvent(new Event("volumechange"));
		});
	...
	get paused() {
		...
		return [
			...
			2,
			5
		].includes((_b = (_a = this.api) == null ? void 0 : _a.getPlayerState) == null ? void 0 : _b.call(_a));
	}
	get duration() {
		var _a, _b;
	...
	get buffered() {
		...
		if (progress > 0) return createTimeRanges(0, progress);
	...
	set currentTime(val) {
		...
		this.loadComplete.then(() => {
			var _a, _b;
			(_a = this.api) == null || _a.seekTo(val, true);
			if (this.paused) (_b = this.#seekComplete) == null || _b.then(() => {
				var _a2;
				if (!this.#seekComplete) return;
				(_a2 = this.api) == null || _a2.pauseVideo();
			});
		});
	}
	...
	set muted(val) {
		...
		this.loadComplete.then(() => {
			var _a, _b;
			val ? (_a = this.api) == null || _a.mute() : (_b = this.api) == null || _b.unMute();
		});
	}
	get muted() {
		var _a, _b;
	...
	set playbackRate(val) {
		...
		this.loadComplete.then(() => {
			var _a;
			(_a = this.api) == null || _a.setPlaybackRate(val);
		});
	}
	get playsInline() {
	...
	set volume(val) {
		...
		this.loadComplete.then(() => {
			var _a;
			(_a = this.api) == null || _a.setVolume(val * 100);
		});
	}
	get volume() {
		var _a;
		if (!this.isLoaded) return this.#initialVolume;
...
if (globalThis.customElements && !globalThis.customElements.get("youtube-video")) globalThis.customElements.define("youtube-video", YoutubeVideoElement);
var youtube_video_element_default = YoutubeVideoElement;
//#endregion
//#region node_modules/youtube-video-element/dist/react.js
...
//# sourceMappingURL=react-DaiFkyYj.js.map